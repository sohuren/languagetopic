...
public class Relation {
    ...
    static final int NUM_RELATIONS = mctestmain.NOREL? 1 : HOW + 1; // ignoring entity match!
//  static final int NUM_RELATIONS = 1; //IMP: comment this if using relations!
    ...
    static HashMap<Integer, Double> getRelationFeatures(Paragraph paragraph, int [] sentNums, int relation) {
        ...
        if(mctestmain.NOREL) return features; //IMP: comment this if using relations!

        if(mctestmain.MIXED_SINGLE_MULTI && sentNums[0] == sentNums[1]) return features; //IMP: comment this if using multi even for same sentence pairs!
    ...
    static HashMap<Integer, Double> getFeaturesQR(Paragraph paragraph, int quesNum, int relation) {
        ...
        if(mctestmain.NOREL || mctestmain.NO_QR_FEATURES) return features;        //IMP: comment this if using relations!
