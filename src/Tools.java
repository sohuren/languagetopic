...
public class Tools {
    ...
    public static int findString(String values, String sentence) {
        
    	int count = 0;
    	int strlen = values.length();
    	
    	for(int i = 0; i < sentence.length() - strlen + 1; i++)
    	{
    		if(values == sentence.substring(i, i + strlen - 1))	count++;  // the length should be strlen, not strlen + 1 
    	}
    ...
    {	
        ...
        vec1 = VectorMul(vec1, 1.0/sentence1.size());
