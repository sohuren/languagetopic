...
public class Relation {
    ...
    static final int NUM_RELATIONS = mctest_main.NOREL? 1 : HOW + 1; // ignoring entity match!
    ...
    static HashMap<Integer, Double> getRelationFeatures(Paragraph paragraph, int [] sentNums, int relation) {
        ...
        if(mctest_main.NOREL) return features; //IMP: comment this if using relations!

        if(mctest_main.MIXED_SINGLE_MULTI && sentNums[0] == sentNums[1]) return features; //IMP: comment this if using multi even for same sentence pairs!
    ...
    static HashMap<Integer, Double> getFeaturesQR(Paragraph paragraph, int quesNum, int relation) {
        ...
        if(mctest_main.NOREL || mctest_main.NO_QR_FEATURES) return features;        //IMP: comment this if using relations!
