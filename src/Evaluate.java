...
public class Evaluate {
    ...
    static boolean BASIC_SCORING= true;
    static boolean SCORE_PAIRS_DIRECT = false;

    static HashMap<Integer, Integer> qType2IncorrectCnt = new HashMap<>();
    ...
    //rank sentences given the correct sentence, sanity check
...
//                        cnt++;
    ...
    static void rankAnswers(Paragraph paragraph) {
        ...
        for(Question Q : paragraph.questions) {
            ...
            double [] ansScores = new double[Q.choices.size()];
...
//    }
    ...
    static double evaluate(ArrayList<Paragraph> paragraphs) {
        ...
        for(Paragraph paragraph : paragraphs) {
            ...
            total += paragraph.questions.size();
            paraNum++;
        ...
        double multiAccuracy = (correctMulti*100)/totalMulti;
        
        System.out.println("Accuracy: " + accuracy);
        System.out.println("Single Accuracy: " + singleAccuracy);
        System.out.println("Multi Accuracy: " + multiAccuracy);
    ...
    static IntPair getAnsScores(Map<Integer, Double> sent2Scores, Paragraph paragraph, int quesNum, int ansNum, double [] ansScores) {
        ...
        ArrayList<Double> numQZ = new ArrayList<>();
        ArrayList<Double> denQZ = new ArrayList<>();
        ...
        if(!MULTI_SENTENCE_EVAL || (mctest_main.MIXED_SINGLE_MULTI && paragraph.questions.get(quesNum).type==1)) {
            for (Map.Entry<Integer, Double> sent : sent2Scores.entrySet()) {
                

            	double[] choiceScore = new double[paragraph.questions.get(quesNum).choices.size()];
                
                for (int choiceNum = 0; choiceNum < paragraph.questions.get(quesNum).choices.size(); choiceNum++)
...
//                ansScores[ansNum] *= -1;
            ...
            for (int z1 = 0; z1 < N; z1++) {
                ArrayList<Double> numQR = new ArrayList<>();
                ArrayList<Double> denQR = new ArrayList<>();

                for(int r = 0 ;r < Relation.NUM_RELATIONS; r++) {

                    ArrayList<Double> numZ2 = new ArrayList<>();
                    ArrayList<Double> denZ2 = new ArrayList<>();

                    for (int z2 : SentRel.getNeighborSentences(z1, N)) {

                        ArrayList<Double> valuesChoice = new ArrayList<>();
...
//        ansScores[ansNum] = bestSentenceScore;
    ...
    static MutablePair<Double, Double> evaluateAnswers(Paragraph paragraph, int paraNum) {
        ...
        for(Question Q : paragraph.questions) {
            ...
            double [] ansScores = new double[Q.choices.size()];
            int []bestSentences = new int[Q.choices.size()];
            int []bestSentenceNeighbors = new int[Q.choices.size()];
