...
public class SentRel {
    ...
    static HashMap<String, Integer> word2Cnt = new HashMap<>(); //word counts from a big corpus like Wikipedia
    ...
    static void initialize() throws IOException, InterruptedException {
        ...
        int i = 0;
        System.err.println("Calculate features ");
        ...
        for(Paragraph paragraph : SentRel.trainParagraphs) {
                ...
                for (int ansNum=0; ansNum < Q.choices.size(); ansNum++) {
                    ...
                    for (int sentNum = 0; sentNum < N; sentNum++) {
                    	...
                    	if (MULTI_SENTENCE) {
                            ...
                            if(mctest_main.MIXED_SINGLE_MULTI)
            ...
            System.err.print("\r" + (i++) + "documents");
        }
        System.err.println();
    ...
    static void readWordVectors() throws IOException, InterruptedException {
    	...
    	BufferedReader br = new BufferedReader(new FileReader(mctest_main.wordVectorFile));
        try {
        ...
        System.err.println("Read" + Integer.toString(wordVec.size()) + " word vectors");
    }

    static void readWordList() throws IOException, InterruptedException {
        BufferedReader br = new BufferedReader(new FileReader(mctest_main.wordListFile));
        try {
            StringBuilder sb;
            String line = br.readLine();
        ...
        System.err.println("Read" + Integer.toString(word2Cnt.size())+" words");
    }

    static void readStopWords() throws IOException, InterruptedException {
        BufferedReader br = new BufferedReader(new FileReader(mctest_main.stopWordsFile));
        try {
        ...
        System.err.println("Read" + Integer.toString(stopWords.size())+" stopwords");
    }
...
//      Tools.updateMap(features, getFeaturesAQZSingle(paragraph, sentNums[0], quesNum, ansNum));  // IMP : using z1 also
    ...
    static HashMap<Integer, Double> getFeaturesQZSingle(Paragraph paragraph, int sentNum, int quesNum) {
        ...
        if(mctest_main.SWD_ONLY) return features; // IMP: use if running with only SWD features
        ...
        if(mctest_main.MIXED_SINGLE_MULTI) qType = Integer.toString(Q.type);
...
//        Tools.updateMap(features, Clause.getFeaturesAQZ(paragraph, sentNum, quesNum, ansNum));
    ...
    static Set<Integer> getNeighborSentences(int sentNum, int N) {
        HashSet<Integer> neighborSents = new HashSet<>();
...
//        }
    ...
    static HashMap<IntPair, Double> sentDist(Paragraph paragraph, int quesNum) {
        ...
        HashMap<IntPair, Double> dist = new HashMap<>();
